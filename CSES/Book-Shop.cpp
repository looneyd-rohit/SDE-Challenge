...
// [IMP]: Locality of reference DP, gives TLE on memoization, but not on tabulation
...
int dp[(int)(1001)][(int)(1e5+1)];

int solve(int index, int x, vector<int>& prices, vector<int>& pages){
	// base case
	if(index == -1){
		// out of bounds
		return 0;
	...
	int dontTake = solve(index-1, x, prices, pages);

	// take this book
	int take = -1e9;
	if(x-prices[index] >= 0){
		take = pages[index] + solve(index-1, x-prices[index], prices, pages);
	}
	return dp[index][x] = max(dontTake, take);
}
	...
	// {
		...
		for(int index=0; index<=n-1; index++){
			for(int xx=0; xx<=x; xx++){
				// don't take this book
				int dontTake = index>0 ? dp[index-1][xx] : 0;

				// take this book
				int take = -1e9;
				if(xx-prices[index] >= 0){
					take = pages[index];
					take += index>0 ? dp[index-1][xx-prices[index]] : 0;
				}
				dp[index][xx] = max(dontTake, take);
			}
		}
		// cout<<solve(n-1, x, prices, pages)<<endl;
		cout<<dp[n-1][x]<<endl;
