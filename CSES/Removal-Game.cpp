...
int dp[(int)(5001)][(int)(5001)][2];

int solve(int i, int j, bool isA, vector<int>& arr){
	// base case
	if(i > j) return 0;
	...
	if(isA){
		// player A's turn
		return dp[i][j][isA] = max(arr[i] + solve(i+1, j, !isA, arr), arr[j] + solve(i, j-1, !isA, arr));
	}else{
		// player B's turn
		return dp[i][j][isA] = min(solve(i+1, j, !isA, arr), solve(i, j-1, !isA, arr));
	}
}
