...
int32_t main() {
  ...
  // while(t--){
    ...
    // dp[i][0] --> time to reach i-th floor (at stairs in the i-th floor)
    // dp[i][1] --> time to reach i-th floor (at elevator in the i-th floor)
    // base case
    // if already at first floor --> at stairs dp[1][0] = 0
    // if already at first floor --> at elevator dp[1][1] = c, time to 
    // get inside elevator
    dp[1][0] = 0, dp[1][1] = c;
    for(int i=2; i<=n; i++){
      // stairs
      ...
      // elevator
      dp[i][1] = min({b[i-1] + dp[i-1][1], b[i-1] + c + dp[i-1][0]});

      // cout<<dp[i][0]<<" and "<<dp[i][1]<<endl;
    }
