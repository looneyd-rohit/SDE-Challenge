...
int32_t main() {
  ...
  ull n;
  cin>>n;
  // approach based on matrix exponentiation
  // construct the transition matrix or length one
  // transition first (base case)
  // States -> happy  sad
  // 	happy |  19	   7 |
  // 	sad   |  6	  20 |
  vector<vector<ull>> transitions(2, vector<ull>(2, 0)), res(2, vector<ull>(2, 0));
  transitions[0][0] = 19, transitions[0][1] = 7, transitions[1][0] = 6, transitions[1][1] = 20;
  res[0][0] = res[1][1] = 1;
  while(n){
  	if(n & 1){
