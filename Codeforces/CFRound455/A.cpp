...
int solve(int index, vector<int>& hash){
	// base case
	if(index <= 0){
		return 0;
	}
	...
	int del = (hash[index] * index) + solve(index-2, hash);


	// don't delete last index, delete previous (second last)
	int noDel = solve(index-1, hash);

	return dp[index] = max(del, noDel);
}
