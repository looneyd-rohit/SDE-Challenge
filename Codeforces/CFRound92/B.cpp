...
int32_t main() {
  ...
  while(t--){
    ...
    int maxScore = 0;
    for(int j=0; j<=z; j++){
      // we are slowly incrementing no of left moves made
      // for each left move made --> total distance travelled right gets
      // decreased by 2
      // also a slight observation: in the total journey we will only
      // go back and forth on one index, if we had to move left at all
      // since consecutive left movements are not allowed, also
      // that back and forth will take place only on the max(a[i]+a[i+1])
      ...
      maxScore = max(maxScore, sum+maxi*j);
    }
    cout<<maxScore<<endl; 
  }
  return 0;
