...
int dp[(int)205][(int)205];

bool isValid(vector<string> &arr, string &s) {
  for (auto &str : arr) {
    int diff = 0;
...
int32_t main() {
  ...
  while (t--) {
    ...
    for (int i = 0; i < n; i++)
      cin >> arr[i];
    // check for 0-th string itself
    if(isValid(arr, arr[0])){
      cout<<arr[0]<<endl;
    }else{
      // try out all the 26 possibilities for each index
      // after taking a particular string
      // since the resulting string varies from every given
      // at only one pos, initially take any one string from
      // given lot and change every index to match the criteria
      string ans = arr[0];
      bool found = false;
      for(int i=0; i<m; i++){
