...
bool isPossible(vector<int>& arr, int n, int k, int minLen){
	// min cuts for any log of L, to make it equal to minLen
	// is ceil(L / minLen) - 1
	int cuts = 0;
	for(int i=0; i<n; i++){
		cuts += ceil((1.0*arr[i]) / minLen) - 1;
	}
	return (cuts <= k);
}
...
int32_t main()
{
	...
	while(low <= high){
		...
		int minLen = mid;
		bool flag = isPossible(arr, n, k, minLen);
		if(flag){
			// if possible try for even smaller, since larger will
			// be obviously possible
			high = mid - 1;
		}else{
			low = mid + 1;
		}
